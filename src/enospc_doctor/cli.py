...
def _print_text(reports, show_all: bool) -> None:
    shown = 0
    for r in reports:
        if not show_all and r.cause == CAUSE_OK:
            continue
        shown += 1
        print(f"\n{r.mountpoint} ({r.filesystem}): {r.cause}")
        print(f"  {r.explanation}")
        print(f"  block use: {r.block_use_pct}%  inode use: {r.inode_use_pct}%")
        if r.reserved_block_pct:
            print(f"  reserved blocks: {r.reserved_block_pct}%")
    ...
    if shown == 0:
        print("No ENOSPC-relevant issues found on any mounted filesystem.")
...
def main(argv=None) -> int:
    ...
    if args.json:
        ...
        _print_text(reports, show_all=args.all)
